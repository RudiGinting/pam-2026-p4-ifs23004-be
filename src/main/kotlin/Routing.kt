...
fun Application.configureRouting() {
    ...
    val obatService: ObatService by inject()
    ...
    routing {
        get("/") {
            call.respondText("API telah berjalan. Dibuat oleh Rudi Alva Jonathan Ginting.")
        }
        ...
        route("/obat") {
            get {
                obatService.getAllObat(call)
            }
            post {
                obatService.createObat(call)
            }
            get("/{id}") {
                obatService.getObatById(call)
            }
            put("/{id}") {
                obatService.updateObat(call)
            }
            delete("/{id}") {
                obatService.deleteObat(call)
            }
            get("/{id}/image") {
                obatService.getObatImage(call)
            }
        }
    }
}